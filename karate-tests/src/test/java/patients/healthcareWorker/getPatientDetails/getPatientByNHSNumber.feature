...
Background:
  ...
  * def defaultNHSNumber = karate.env == 'mock' ? '9000000009' : '9693632109'
  * url baseURL

@get
Scenario: Get a patient, validate the schema
  * def nhsNumber = karate.get('nhsNumber', defaultNHSNumber)
  ...
  * match response == read('classpath:patients/healthcareWorker/patient-response-template.json')
