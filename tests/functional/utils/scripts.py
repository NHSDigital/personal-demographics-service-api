...
class PDS:
    ...
    @staticmethod
    # A function to send a PDS Retrieve request. Arguments accepted are the Patient_ID & Header.  It returns the response to the calling Test/Function.
    ...
    @staticmethod
    # A function to send a PDS Retrieve request. Arguments accepted are the Query Parameters & Header.  It returns the response to the calling Test/Function.
    ...
    @staticmethod
    # A function to send a PDS Update request.  Argument accepted are Patient_ID, Patients record version, Patch Payload, and any additional Headers. It returns the response to the calling Test/Function.
    ...
    @staticmethod
    # A function to send a PDS Update request where the standard headers need to be amended. Argument accepted are Patient_ID, Patch Payload, and any additional Headers. It returns the response to the calling Test/Function.
    def update_patient_invalid_headers(patient, payload, headers=None):
        response = requests.patch(f"{config.PDS_BASE_PATH}/Patient/{patient}", headers=headers, json=payload)
        return response

    @staticmethod
    # A function to send a PDS Retrieve Related Person request. Arguments accepted are the Patient_ID & Header.  It returns the response to the calling Test/Function.
    ...
    @staticmethod
    #  A Function to check the Response Body of an Update Request.  Arguments accepted are the actual Response & expected Response.
    ...
    @staticmethod
    #  A Function to check the Response Body of a Search & RelatedPerson Request.  Arguments accepted are the actual Response & expected Response.
    ...
    def check_response_headers(response, expected_headers={}):
        if 'X-Request-ID' in expected_headers:
            with check: assert response.headers['X-Request-ID'] == expected_headers['X-Request-ID'], f"UNEXPECTED RESPONSE: " \
                                                                                                     f"actual X-Request-ID is: {response.headers['X-Request-ID']} " \
                                                                                                     f"expected X-Request-ID is: {expected_headers['X-Request-ID']}"
        else:
            with check: assert 'X-Request-ID' not in response.headers, f"UNEXPECTED RESPONSE: expected X-Request-ID not to be present " \
                                                                       f"but {response.headers['X-Request-ID']} found in response header"

        if 'X-Correlation-ID' in expected_headers:
            with check: assert response.headers['X-Correlation-ID'] == expected_headers['X-Correlation-ID'], f"UNEXPECTED RESPONSE: " \
                                                                                                             f"actual X-Request-ID is: {response.headers['X-Correlation-ID']} " \
                                                                                                             f"expected X-Request-ID is: {expected_headers['X-Correlation-ID']}"

        else:
            with check: assert 'X-Correlation-ID' not in response.headers, f"UNEXPECTED RESPONSE: expected X-Correlation-ID not to be present " \
